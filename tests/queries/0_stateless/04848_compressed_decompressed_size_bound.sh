...
# CompressionCodecNone is the only codec reporting isNone() on 26.3, so the shortcut has one carrier.

# The comparison is for inequality, so arms that only ever declare more than the body pin one side of
# it: narrowed to "body shorter than declared", the frame above would still be refused. This one
...
${CLICKHOUSE_CLIENT} --query "
    ...
    -- 26.3 has no packed skip-index archive, so the index always has its own file, which is the
    -- skip-index class checkDataPart iterates over with a per-file checksum entry.
